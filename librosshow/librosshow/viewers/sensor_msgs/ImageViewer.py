...
class ImageViewer(object):
    ...
    def draw(self):
        ...
        if isinstance(self.image.data, list):
            data = np.asarray(self.image.data, np.uint8)
        else:
            data = self.image.data
        if self.image.encoding == 'bgr8':
            current_image = np.frombuffer(data, np.uint8).reshape((self.image.height, self.image.width, 3))[:, :, ::-1]
        elif self.image.encoding == 'rgb8':
            current_image = np.frombuffer(data, np.uint8).reshape((self.image.height, self.image.width, 3))
        elif self.image.encoding == 'mono8' or self.image.encoding == '8UC1':
            current_image = np.frombuffer(data, np.uint8).reshape((self.image.height, self.image.width))
            ...
            current_image = np.frombuffer(data, np.uint16).reshape((self.image.height, self.image.width)).astype(np.float)
